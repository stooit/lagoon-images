...
if [ "$1" = 'mysqld' -a -z "$wantHelp" ]; then
  ...
  if [ -d ${MARIADB_DATA_DIR:-/var/lib/mysql}/mysql ]; then
    ...
    /usr/sbin/mysqld --initialize-insecure --skip-name-resolve --datadir=${MARIADB_DATA_DIR:-/var/lib/mysql} --basedir=/usr
...
EOF
    ...
    if [ "$MARIADB_DATABASE" != "" ]; then
      ...
      if [ "$MARIADB_USER" != "" ]; then
        ...
        echo "CREATE USER '$MARIADB_USER'@'%' IDENTIFIED BY '$MARIADB_PASSWORD';" >> $tfile
        echo "GRANT ALL ON \`$MARIADB_DATABASE\`.* to '$MARIADB_USER'@'%';" >> $tfile
      fi
    fi
    ...
    if [ "$(find /docker-entrypoint-initdb.d -mindepth 1 -maxdepth 1 | read)" ]; then
      echo "entrypoint not empty"
